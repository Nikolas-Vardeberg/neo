...
export const useMoveVideos = (videoId: string, currentWorkspace: string) => {
    ...
    const { mutate, isPending } = useMutationData(["change-video-location"], (data: { folder_id: string; workspace_id: string }) => moveVideoLocation(videoId, data.workspace_id, data.folder_id))

    const { errors, onFormSubmit, register, watch } = useZodForm(moveVideoSchema, mutate, { folder_id: null, workspace_id: currentWorkspace })
